...
class NumberFormatTest {
    ...
    fun formatterHex() {
        ...
        val formatState = CalcViewModel.FormatState(1e-4, 2, format)
        val formatter = format.formatter()
        assertEquals(AnnotatedString("0 = 0x0"), formatter.format(0.0, formatState))
        assertEquals(AnnotatedString("1 = 0x1"), formatter.format(1.0, formatState))
        assertEquals(AnnotatedString("-1 = 0xffffffffffffffff"), formatter.format(-1.0, formatState))
        assertEquals(AnnotatedString("1.5 = 0x1 + ϵ"), formatter.format(1.5, formatState))
        assertEquals(AnnotatedString("16 = 0x10"), formatter.format(16.0, formatState))
        assertEquals(AnnotatedString("-16 = 0xfffffffffffffff0"), formatter.format(-16.0, formatState))
    }
    ...
    fun formatterPrime() {
        ...
        assertEquals(AnnotatedString("1.5 = 1"), formatter.format(3/2.0, formatState))
        assertEquals(AnnotatedString("-1 = -1"), formatter.format(-1.0, formatState))
    ...
    fun formatterTime() {
        ...
        val formatState = CalcViewModel.FormatState(1e-4, 2, format)
        val formatter = format.formatter()
        assertEquals(AnnotatedString("0 = 0:00"), formatter.format(0.0, formatState))
